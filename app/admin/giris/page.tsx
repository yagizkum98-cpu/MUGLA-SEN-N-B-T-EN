...
import {isAdminAuthorityDomain, isSuperAdminDomain, municipalityUrl} from '@/lib/domain-routing'
  ...
  return <main className="grid min-h-screen bg-mugla-sand lg:grid-cols-[.9fr_1.1fr]">
    <section className="hidden bg-mugla-navy p-14 text-white lg:flex lg:flex-col">
      ...
      <div className="my-auto max-w-xl">
        ...
        <h1 className="mt-8 text-5xl font-bold leading-tight">{isSuperAdminDomain() ? 'Super admin merkezi tum portallari tek otoriteden kontrol eder.' : 'Belediye paneli yalnizca tanimli yetkililere aciktir.'}</h1>
        <p className="mt-5 text-lg leading-8 text-white/60">{isSuperAdminDomain() ? 'Belediye yonetim paneli, vatandas paneli, veriler, roller ve kritik baglantilar burada toplanir.' : 'Super admin, admin ve yetkili rollerinden biri tanimli olmayan kullanici bu panele giremez.'}</p>
      </div>
    </section>
    ...
    <section className="grid place-items-center p-6">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={submit} className="mt-7 space-y-4">
          ...
          <Button type="submit" variant="orange" disabled={loading} className="h-14 w-full"><LockKeyhole size={18}/>{loading ? 'Giris yapiliyor...' : isSuperAdminDomain() ? 'Super admin paneline gir' : 'Belediye paneline gir'}</Button>
        </form>
      </div>
    </section>
  </main>
