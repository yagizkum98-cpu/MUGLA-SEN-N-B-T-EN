...
export function CitizenDashboard() {
  ...
  const hasLiveOwnProject = myProjects.some(project => ['Oylamada', 'Yılın Kazanan Adayı'].includes(String(project.status)))
      ...
      <Card id="sepetim">
        ...
        <CardContent className="grid gap-6 xl:grid-cols-[1fr_1fr]">
          <section className="space-y-3">
            ...
            {basketProjects.length ? basketProjects.map(project => <div key={project.id} className="fade-up-card flex flex-wrap items-center gap-3 rounded-2xl border border-mugla-navy/10 p-4"><span className="h-12 w-2 rounded-full" style={{background: project.color}}/><div className="min-w-0 flex-1"><p className="font-bold">{project.title}</p><div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-mugla-navy/50"><span>{project.district}</span><CategoryBadge label={projectCategoryLabel(project)} color={project.color}/></div></div><button type="button" aria-label={`${project.title} projesini sepetten çıkar`} onClick={() => remove(project.id)} className="grid h-10 w-10 place-items-center rounded-full border border-red-100 bg-red-50 text-red-700 hover:bg-red-100"><Trash2 size={17}/></button></div>) : <div className="rounded-2xl border border-dashed border-mugla-navy/20 p-8 text-center text-mugla-navy/45"><ShoppingCart className="mx-auto mb-3"/><p className="font-semibold">Sepetiniz boş.</p><p className="mt-1 text-sm">Oylamadaki projelerden en fazla kalan krediniz kadar seçim yapabilirsiniz.</p></div>}
            <p className="rounded-2xl bg-mugla-sand/70 p-3 text-sm font-semibold text-mugla-navy/60">Onaylarsanız {basketProjects.length} kredi düşer; kalan kredi {Math.max(0, remaining - basketProjects.length)} olur.</p>
          ...
          <section className="space-y-3">
            ...
            {activeVoteProjects.length ? activeVoteProjects.map(project => {
              ...
              return <div key={project.id} className="fade-up-card flex flex-wrap items-center gap-3 rounded-2xl border border-mugla-navy/10 p-4"><span className="h-12 w-2 rounded-full" style={{background: project.color}}/><div className="min-w-0 flex-1"><p className="font-bold">{project.title}</p><div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-mugla-navy/50"><span>{project.district}</span><CategoryBadge label={projectCategoryLabel(project)} color={project.color}/><span>Canlı sonuçlar gizli</span></div></div>{done ? <span className="rounded-full bg-green-50 px-3 py-2 text-xs font-bold text-green-700">Oy alındı</span> : selected ? <button type="button" aria-label={`${project.title} projesini sepetten çıkar`} onClick={() => remove(project.id)} className="grid h-10 w-10 place-items-center rounded-full border border-red-100 bg-red-50 text-red-700 hover:bg-red-100"><Trash2 size={17}/></button> : <Button type="button" size="sm" variant="orange" disabled={basket.length >= remaining} onClick={() => addToBasket(project.id)}><Plus size={15}/> Ekle</Button>}</div>
