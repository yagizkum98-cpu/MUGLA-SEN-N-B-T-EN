...
export function AdminAuthGate({children}: {children: React.ReactNode}) {
  ...
  if (!checked) return <main className="grid min-h-screen place-items-center bg-mugla-sand p-6">
    <p className="font-semibold text-mugla-navy/55">Admin oturumu kontrol ediliyor...</p>
  </main>
  ...
  if (!admin) return <main className="grid min-h-screen place-items-center bg-mugla-sand p-6">
    <section className="w-full max-w-lg rounded-3xl bg-white p-8 text-center shadow-soft">
      ...
      <p className="mt-6 text-xs font-bold tracking-[.2em] text-mugla-orange">ADMIN GIRISI GEREKLI</p>
      ...
      <Link href="/admin/giris" className="mt-7 inline-flex"><Button variant="orange"><LockKeyhole size={17}/> Admin girisi</Button></Link>
    </section>
  </main>
