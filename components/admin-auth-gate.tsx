...
import {isAdminAuthorityDomain, isSuperAdminDomain, municipalityUrl, superAdminUrl} from '@/lib/domain-routing'
...
export function AdminAuthGate({children}: {children: React.ReactNode}) {
  ...
  if (!admin) return <main className="grid min-h-screen place-items-center bg-mugla-sand p-6">
    <section className="w-full max-w-lg rounded-3xl bg-white p-8 text-center shadow-soft">
      ...
      <p className="mt-6 text-xs font-bold tracking-[.2em] text-mugla-orange">{isSuperAdminDomain() ? 'SUPER ADMIN GIRISI GEREKLI' : 'BELEDIYE PANELI GIRISI GEREKLI'}</p>
